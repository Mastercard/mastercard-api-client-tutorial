  ...
  class TokenDetailDataGetTokenOnly
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TokenDetailDataGetTokenOnly` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TokenDetailDataGetTokenOnly`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'payment_account_reference')
    ...
    def list_invalid_properties
      ...
      if !@token_number.nil? && @token_number.to_s.length > 19
        invalid_properties.push('invalid value for "token_number", the character length must be smaller than or equal to 19.')
      end

      if !@token_number.nil? && @token_number.to_s.length < 9
        invalid_properties.push('invalid value for "token_number", the character length must be great than or equal to 9.')
      end
      ...
      if !@payment_account_reference.nil? && @payment_account_reference.to_s.length > 29
        invalid_properties.push('invalid value for "payment_account_reference", the character length must be smaller than or equal to 29.')
      end
    ...
    def valid?
      return false if !@token_number.nil? && @token_number.to_s.length > 19
      return false if !@token_number.nil? && @token_number.to_s.length < 9
      return false if !@expiry_month.nil? && @expiry_month.to_s.length > 2
      return false if !@expiry_year.nil? && @expiry_year.to_s.length > 2
      return false if !@payment_account_reference.nil? && @payment_account_reference.to_s.length > 29
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] token_number Value to be assigned
    def token_number=(token_number)
      if !token_number.nil? && token_number.to_s.length > 19
        fail ArgumentError, 'invalid value for "token_number", the character length must be smaller than or equal to 19.'
      end

      if !token_number.nil? && token_number.to_s.length < 9
        fail ArgumentError, 'invalid value for "token_number", the character length must be great than or equal to 9.'
      end
    ...
    def payment_account_reference=(payment_account_reference)
      if !payment_account_reference.nil? && payment_account_reference.to_s.length > 29
        fail ArgumentError, 'invalid value for "payment_account_reference", the character length must be smaller than or equal to 29.'
      end
    ...
    def hash
      [token_number, expiry_month, expiry_year, payment_account_reference].hash
    end
